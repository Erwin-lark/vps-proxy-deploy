...
## v4.3.1 — DNS-01 无防火墙管理修复 (2026-08-26)

### 修复

- 修复 `MANAGE_UFW_ENV=0` 与 DNS-01 组合下的退出状态：脚本原先仅跳过 UFW 修改，却会因条件提示语的返回值错误中止在系统准备阶段。现在该分支明确成功返回；不会修改防火墙。
- 回归测试新增 DNS-01、保持防火墙不变时的成功返回断言。

### 兼容性

- 不改变协议、端口、域名、Cloudflare Tunnel、订阅 URL、UUID、密码、Reality 密钥或状态字段。v4.3.0 已中止的主机可直接以原参数重跑 v4.3.1。

---

## v4.3.0 — 恢复服务商代码节点命名 (2026-08-26)

### 接口与节点命名

- 节点名称统一为“国旗 + 国家码 + 服务商代码 + 协议缩写”，例如 `🇯🇵 JP BVL VR`；Clash、Loon、Quantumult X、Clash 策略组和订阅门户全部使用同一前缀。
- 交互安装新增服务商代码提示；非交互位置参数继续保持 `DOMAIN EMAIL`，服务商改由 `PROVIDER_ENV` 提供，避免再次破坏现有自动化的位置参数顺序。
- 服务商代码接受 2–8 位字母或数字并统一转成大写，例如 `GCP`、`ALI`、`YOO`、`BVL`。不根据 IP 自动猜测服务商，因为公网 ASN/组织信息不能可靠识别转售商或品牌。
- 首次非交互安装缺少或传入无效 `PROVIDER_ENV` 时，在系统修改前明确停止；已有合法 `STATE_PROVIDER` 时安全重跑会自动复用，也可显式传入新代码完成更名。

### 状态迁移与兼容性

- 新状态文件写入 `STATE_PROVIDER`。v3 `subs.conf` 中合法的旧 `PROVIDER` 会经过大写规范化和格式校验后迁移；不合法历史值不会被信任。
- v4.2.x 状态没有服务商字段，升级时必须先以安装模式传入一次 `PROVIDER_ENV`；迁移前直接执行新版 `--check` 会给出可操作提示，不会生成缺少服务商的名称。
- 更名只重写生成配置中的显示名称；Reality、Hysteria2、XHTTP、WebSocket 和 Quantumult X UUID、密码、秘密路径、Reality 密钥及订阅 Token 全部复用，订阅 URL 不变。客户端需要手动刷新远程资源才能看到新名称。
- 本版根据当前明确需求恢复 v4.1.2 删除的服务商代码。v4.1.2 记录继续作为历史操作保留，但不再被视为当前正确设计。

### 验证与文档

- 回归覆盖服务商格式边界、小写规范化、非交互必填、旧状态复用、v3 迁移、四协议跨客户端命名、Clash 策略组、订阅门户，以及“更名不改变 Quantumult X UUID”；完整回归为 60 项。
- README 和超详细手册同步增加首次安装、macOS 钥匙串安全传 Token、v4.2.x 升级、客户端刷新、404 排查与最终验收中的服务商步骤。
